...
class PersonalWelcome < ActiveRecord::Base
  ...
  def self.deliver
    projects = Project.find(:all, :conditions => "created_at > '#{Time.now.advance :days => -30}' AND owner_id not in (select user_id from personal_welcomes)").group_by{|p| p.owner_id}
