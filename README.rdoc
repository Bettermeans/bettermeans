...
#for my sql development (not recommended)
rake db:create
rake db:schema:load
#rake db:seed (coming soon)
rake db:migrate
...
rake db:schema:dump 
rake db:seed:dump MODELS=tracker,issue_priority,issue_status,enumeration,role,plan,setting
# Uses https://github.com/rhalff/seed_dump

To generate a pg_dump:
pg_dump bettermeans_development -c -O -x -f db/pgdump.sql
